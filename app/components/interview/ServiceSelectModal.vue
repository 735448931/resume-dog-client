    ...
    <UModal v-model:open="isOpen" :ui="{ content: 'max-w-[880px]' }" @after-leave="onClose">
        ...
        <template #body>
            <div class="flex flex-col gap-3 p-1">
                <div v-for="item in serviceHighlights" :key="item.id"
                    ...
                    <!-- 选中角标 -->
                    <div v-if="selectedService === item.id"
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span
                                class="text-[10px] px-1.5 py-0.5 rounded-full bg-primary-100 text-primary-600 font-medium">{{
                                    item.badge }}</span>
                        </div>
                    </div>
                    ...
                    <ul class="flex flex-col gap-1.5">
                        <li v-for="point in item.points" :key="point"
                            ...
                            <UIcon name="i-heroicons-check-circle"
                                class="w-3.5 h-3.5 text-primary-400 mt-0.5 shrink-0" />
                            <span>{{ point }}</span>
                        </li>
                    </ul>
